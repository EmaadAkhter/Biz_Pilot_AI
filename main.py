...
app = FastAPI(title="BizPilot AI Backend - Azure Storage")
security = HTTPBearer()
...
def root():
    ...
    return {"message": "BizPilot AI Backend - Azure Blob Storage Enabled"}
...
async def upload_sales(
    ...
    """Upload sales data CSV/Excel file to Azure Blob Storage"""
    try:
...
def get_analytics(
    ...
    """Get comprehensive analytics for uploaded sales data from Azure Blob Storage"""
    try:
        blob_name = get_file_path(filename, current_user["id"])
        analytics = analyze_sales_data(blob_name)
        return analytics
    except HTTPException:
        raise
    except Exception as e:
...
def list_files(current_user: dict = Depends(get_current_user)):
    """List all uploaded files for current user from Azure Blob Storage"""
    try:
...
def delete_file(
    ...
    """Delete a user's uploaded file from Azure Blob Storage"""
    try:
